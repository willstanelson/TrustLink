...
const POLL_INTERVAL_DB   = 8_000;   // ms – how often to poll DB orders
const POLL_INTERVAL_CHAIN = 60_000; // ms – how often to poll chain state
...
/** Validate an email string without executing it */
function isValidEmail(email: string): boolean {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim());
}

/** Sanitize a free-form string – strip control chars, limit length */
function sanitize(value: string, maxLen = 200): string {
  return value.replace(/[\x00-\x1f\x7f]/g, '').slice(0, maxLen).trim();
}

/**
 * Validate that a redirect URL comes from Paystack only.
 * Prevents open-redirect attacks from a compromised API response.
 */
...
function MainDashboard() {
  const { supabase, sessionReady, sessionLoading, sessionError, refreshSession } =
    useAuth();
  const { login, authenticated, user, logout, linkEmail, getAccessToken } =
    usePrivy();

  // Banks ── fetched once on mount
  ...
  const showToast = useCallback(
    (message: string, type: 'success' | 'error' | 'info' | 'warning' = 'success') => {
      // Sanitise message so injected content can't trick the user
  ...
  // Keep stable refs so callbacks inside effects never go stale
  ...
  // ── Set active wallet – guard against unnecessary calls
  ...
  const { data: usdcAllowance, refetch: refetchAllowance } = useReadContract({
    ...
    args:         userAddress
      ...
      : undefined,
    query: {
      enabled: !!userAddress && selectedAsset.symbol === 'USDC',
    },
  });
  ...
  // Build a stable list of IDs (newest first, cap at 20)
  ...
  // Auto-fill buyer email from linked account
  useEffect(() => {
    const activeEmail =
      user?.email?.address ||
      user?.google?.email  ||
  ...
  const resolveBankAccount = useCallback(
    async (account: string, bank: string) => {
      setIsResolving(true);
      setResolveError('');
      setAccountName('');

      // Test-mode bypass (useful for dev / staging)
  ...
  // Trigger resolution when both fields are populated
  useEffect(() => {
    // Skip if this change was caused by auto-fill (avoids a redundant API call)
    if (autoFilled) {
      setAutoFilled(false);
      return;
    }
  ...
  // NOTE: intentionally omitting `autoFilled` and `resolveBankAccount` from deps
  // to avoid re-triggering on those changes; the guard at the top handles it.

  // Auto-fill seller bank details when seller email resolves
  useEffect(() => {
    if (mode !== 'fiat') return;
  ...
  useEffect(() => {
    ...
    fetch('/api/paystack/verify', {
      ...
      .catch((err) => {
        if (!cancelled)
          showToastRef.current(err.message || 'Verification error', 'error');
      });

    return () => { cancelled = true; };
  }, [searchParams, supabase, sessionReady, fetchDbOrders, router]);

  // ── Network-alert badge computation
  ...
  // ── Order list computation  (O(n) instead of O(n²))
  const { myBuyingOrders, mySellingOrders } = useMemo(() => {
    const buying:  any[] = [];
    const selling: any[] = [];

    // Build an O(1) lookup of DB orders by a composite key
    // key = `${buyerLower}|${sellerLower}|${amount}`
    ...
    if (escrowsData && userAddress) {
      escrowsData.forEach((result, index) => {
        ...
        const isAccepted = dbOrder?.status === 'accepted' ||
        ...
        const orderType =
          dbOrder?.trade_type === 'GIFT_CARD' ? 'GIFTCARD' : 'CRYPTO';
        ...
        const order = {
          ...
          isAccepted,
          isShipped,
          isDisputed:    chainDisputed,
          isCompleted:   chainCompleted,
          status,
          statusColor,
    ...
    Object.values(dbOrders).forEach((dbOrder: any) => {
      ...
      if (['success', 'completed'].includes(currentStatus))
        fiatStatusColor = 'bg-slate-700 text-slate-300';
      else if (currentStatus === 'disputed')
        fiatStatusColor = 'bg-red-500/20 text-red-400';
      else if (currentStatus === 'shipped')
        fiatStatusColor = 'bg-blue-500/20 text-blue-400';
      else if (
        ['accepted', 'partially_released', 'processing_payout', 'secured'].includes(
          currentStatus,
        )
      )
  ...
  // ── Resolve seller address (shared between crypto + giftcard)
  async function resolveSellerAddress(raw: string): Promise<string | null> {
    ...
    if (isValidEmail(trimmed)) {
      ...
      try {
        ...
        showToast(
          `Found wallet: ${data.address.slice(0, 6)}...${data.address.slice(-4)}`,
          'success',
        );
    ...
    showToast('Invalid Wallet Address or Email', 'error');
    return null;
  }
  ...
  const handleCryptoTransaction = async () => {
    if (isUnsupportedNetwork) {
      showToast('Please switch to a supported network first.', 'error');
      return;
    }
    ...
    if (finalSellerAddress.toLowerCase() === userAddress?.toLowerCase()) {
      showToast('You cannot create an order with yourself.', 'error');
      return;
    }
    ...
    try {
      ...
      showToast(err.shortMessage || err.message || 'Transaction Error', 'error');
    } finally {
      setIsWriting(false);
    }
  };
  ...
  const handleFiatTransaction = async () => {
    ...
    if (!accountName) {
      showToast('Please wait for bank verification to complete', 'error');
      return;
    }
    ...
    try {
      ...
      // ✅ Validate the redirect URL before sending the user there
      const authUrl = data?.data?.authorization_url;
      if (!isSafePaystackUrl(authUrl)) {
  ...
  const handleGiftCardTransaction = async () => {
    if (trustLevel === null || trustLevel < 3) {
      showToast('Level 3 required to use Gift Card escrow.', 'error');
      return;
    }
    if (!gcSellerAddress.trim()) {
      showToast("Please enter the seller's address or email.", 'error');
      return;
    }
    ...
    const finalSellerAddress = await resolveSellerAddress(gcSellerAddress);
    if (!finalSellerAddress) return;
    ...
    try {
      ...
      const isNative    = selectedAsset.type === 'native';
      const amountWei   = parseUnits(gcAmount, selectedAsset.decimals);
      const symbolLabel = isNative ? activeChain.nativeSymbol : 'USDC';
      ...
      if (!isNative) {
        const currentAllowance = usdcAllowance
          ...
          : BigInt(0);
        if (currentAllowance < amountWei) {
          showToast('Approving USDC for Gift Card… Please wait.', 'info');
          const approveHash = await walletClient.writeContract({
            address:      selectedAsset.address as `0x${string}`,
            abi:          ERC20_ABI,
            functionName: 'approve',
      ...
      if (receipt.status === 'success') {
        const gcSellerEmailIfProvided = isValidEmail(gcSellerAddress.trim())
        ...
        const res  = await fetch('/api/escrow/create', {
          ...
          body:    JSON.stringify({
            ...
            seller_address:       finalSellerAddress,
            buyer_email:          buyerEmail || null,
            seller_email:         gcSellerEmailIfProvided,
            amount:               gcAmount,
            token_symbol:         symbolLabel,
            network:              activeChain.name,
            status:               'secured',
        ...
        if (data.status === 'error') {
          ...
          showToast('Gift Card Escrow Created Successfully!', 'success');
          if (gcSellerEmailIfProvided) {
            sendEmailNotification(
              gcSellerEmailIfProvided,
              `New Gift Card Order on ${activeChain.name}! 🎁`,
              `Great news! A buyer has securely locked ${gcAmount} ${symbolLabel} in TrustLink for a Gift Card trade.\n\nPlease log in to your TrustLink dashboard to provide the Gift Card code securely.`,
            );
          }
        }
        setGcSellerAddress('');
  ...
  const hasEmailLinked =
    !!(user?.email?.address || user?.google?.email || user?.apple?.email || user?.discord?.email);
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-800 border border-emerald-500/30 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center flex flex-col items-center">
            ...
            <p className="text-slate-400 mb-8 leading-relaxed">
              Your fiat payment has been securely locked in escrow. The seller
      ...
      <nav className="flex items-center justify-between px-6 py-6 max-w-6xl mx-auto w-full gap-4">
        ...
        {authenticated ? (
          <div className="flex items-center gap-3 min-w-max">
            ...
            <div className="relative" ref={networkDropdownRef}>
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2.5 rounded-2xl border transition-all text-xs font-bold ${
                    ...
                    return (
                      ...
                      >
                        ...
                        {networkAlerts[chainIdNum] > 0 && (
                          <span className="flex items-center justify-center bg-red-500 text-white text-[10px] font-black w-4 h-4 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 px-4 py-2.5 rounded-2xl transition-all shadow-lg"
            >
            ...
            <button
              ...
              aria-label="Log out"
              onClick={logout}
              className="bg-red-500/10 hover:bg-red-500/20 text-red-400 p-2.5 rounded-2xl border border-red-500/20 transition-all"
            >
          ...
          <button
            type="button"
            onClick={login}
            className="bg-emerald-600 hover:bg-emerald-500 px-6 py-2.5 rounded-2xl text-sm font-bold transition-all shadow-lg shadow-emerald-500/20"
          >
            Log In
          </button>
        )}
        ...
        <div className="w-full max-w-md bg-slate-800/50 border border-slate-700 p-8 rounded-2xl backdrop-blur-sm shadow-2xl relative z-10">
          {!authenticated ? (
            <button
              type="button"
              onClick={login}
              className="w-full bg-emerald-500 hover:bg-emerald-400 py-3 rounded-xl font-bold"
            >
              Connect Wallet
            </button>
          ) : (
            <div className="flex flex-col gap-4">
              ...
              {mode === 'crypto' && (
                <>
                  ...
                  <div>
                    <label
                      htmlFor="crypto-seller"
                      className="text-xs text-slate-400 ml-1 font-bold"
                    >
                  ...
                  <button
                    ...
                    disabled={
                      isWriting ||
                      (!isUnsupportedNetwork &&
                        (!sellerAddress.trim() || !amountInput))
                    }
                  ...
                  {!hasEmailLinked ? (
                    <div className="flex flex-col items-center justify-center p-6 bg-slate-900/50 border border-slate-700 rounded-2xl text-center gap-4 mt-2">
                      ...
                      <p className="text-sm text-slate-400 leading-relaxed">
                        To use Bank Transfer escrows, you must link an email address to
                        your account. This ensures you receive payment receipts and
                        secure dispute notifications.
                      </p>
                      <button
                        type="button"
                        onClick={linkEmail}
                    ...
                    <>
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="col-span-2">
                          <label htmlFor="fiat-amount" className="sr-only">
                            Amount in NGN
                          </label>
                          <input
                            id="fiat-amount"
                      ...
                      <div className="opacity-70 cursor-not-allowed">
                        <label
                          htmlFor="fiat-buyer-email"
                          className="text-xs text-slate-400 ml-1 font-bold flex items-center gap-1.5"
                        >
                      ...
                      <div>
                        <label
                          htmlFor="fiat-seller-email"
                          className="text-xs text-emerald-400 ml-1 font-bold"
                        >
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-2 mt-1">
                          <label htmlFor="fiat-bank" className="sr-only">
                            Select Bank
                          </label>
                          <select
                            id="fiat-bank"
                          ...
                          <div className="relative">
                            <label htmlFor="fiat-acct-no" className="sr-only">
                              Account Number
                            </label>
                            <input
                              id="fiat-acct-no"
                      ...
                      <button
                        ...
                        disabled={
                          !fiatAmount || !accountName || !buyerEmail || !sellerEmail
                        }
                      ...
                      {trustLevel === null ? (
                        ...
                        <>
                          ...
                          <p className="text-sm text-slate-300">
                            Gift Card trading is locked to prevent fraud. Complete more
                            standard trades to reach Trust Level 3 and unlock this
                            feature.
                          </p>
                        </>
                      )}
