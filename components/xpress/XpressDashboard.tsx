...
export default function XpressDashboard() {
  ...
  const handleCryptoTransaction = async () => {
    ...
    try {
      ...
      if (receipt.status === 'success') {
        ...
        setSellerAddress(''); setAmountInput(''); handleRefresh();
  ...
  if (sessionError) {
    return (
      <div className="flex flex-col items-center justify-center p-20">
        ...
        <button type="button" onClick={refreshSession} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-emerald-400 rounded-xl font-bold transition-all shadow-lg flex items-center gap-2">
  ...
  return (
    <div className="w-full text-white font-sans pb-20 relative">
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-800 border border-emerald-500/30 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center flex flex-col items-center">
            ...
            <button type="button" onClick={() => setShowSuccessModal(false)} className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-extrabold text-lg py-4 rounded-xl transition-all shadow-lg">
              View My Order
            </button>
          </div>
        </div>
      ...
      {notification && (
        <div role="alert" aria-live="polite" className={`fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-4 rounded-xl shadow-2xl border backdrop-blur-md max-w-sm ${notification.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : notification.type === 'info' ? 'bg-blue-500/10 border-blue-500/50 text-blue-400' : notification.type === 'warning' ? 'bg-amber-500/10 border-amber-500/50 text-amber-400' : 'bg-red-500/10 border-red-500/50 text-red-400'}`}>
      ...
      {/* ── XPRESS CONTEXT NAV BAR ── */}
      <nav className="flex items-center justify-between py-6 max-w-6xl mx-auto w-full gap-4">
        {/* We removed the redundant TrustLink logo here since the Shell handles it */}
        
        {authenticated && (
          <form onSubmit={handleSearch} className="flex-1 max-w-lg relative group" role="search">
            ...
            <input
              type="search" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} autoComplete="off" aria-label="Search seller by email or wallet"
              className="block w-full pl-10 pr-3 py-2.5 border border-slate-700 rounded-2xl bg-slate-900/50 text-slate-300 placeholder-slate-500 focus:outline-none focus:bg-slate-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 sm:text-sm transition-all shadow-inner"
        ...
        {authenticated ? (
          <div className="flex items-center gap-3 min-w-max">
            <div className="relative" ref={networkDropdownRef}>
              <button type="button" aria-label="Select network" aria-expanded={isNetworkListOpen} onClick={() => setIsNetworkListOpen((v) => !v)} className={`flex items-center gap-2 px-3 py-2.5 rounded-2xl border transition-all text-xs font-bold ${isUnsupportedNetwork ? 'bg-red-500/10 border-red-500 text-red-400' : 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700'}`}>
                <Globe className="w-4 h-4" aria-hidden />
                <span className="hidden sm:block">{isUnsupportedNetwork ? 'Unsupported' : activeChain.name}</span>
                {totalActionableOrders > 0 && <span aria-label={`${totalActionableOrders} actions required`} className="flex items-center justify-center bg-red-500 text-white text-[10px] font-black w-4 h-4 rounded-full animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.8)]">{totalActionableOrders}</span>}
              ...
              {isNetworkListOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-xl z-[100] overflow-hidden shadow-xl">
                  {Object.entries(CHAIN_CONFIG).map(([id, config]) => {
                    ...
                    return (
                      <button key={id} type="button" onClick={() => { switchChain({ chainId: chainIdNum }); setIsNetworkListOpen(false); }} className={`w-full text-left px-4 py-3 text-sm font-bold hover:bg-slate-700 transition-colors flex items-center justify-between ${chainIdNum === chainId ? 'text-emerald-400 bg-slate-700/50' : 'text-slate-300'}`}>
                        <span>{config.name}</span>
                        {networkAlerts[chainIdNum] > 0 && <span className="flex items-center justify-center bg-red-500 text-white text-[10px] font-black w-4 h-4 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]">{networkAlerts[chainIdNum]}</span>}
                      </button>
                    );
                  })}
                </div>
            ...
            <button type="button" aria-label="Open wallet" onClick={() => setIsWalletModalOpen(true)} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 px-4 py-2.5 rounded-2xl transition-all shadow-lg">
              <div className="flex flex-col items-start">
                <span className="font-mono text-sm font-bold truncate max-w-[100px] sm:max-w-[150px]">
                  {activeEmail ? activeEmail.split('@')[0] : userAddress ? `${userAddress.slice(0, 6)}…${userAddress.slice(-4)}` : 'Wallet'}
                </span>
            ...
            <Link href="/profile" aria-label="Profile settings" className="flex items-center justify-center p-2.5 bg-slate-800 border border-slate-700 rounded-2xl hover:bg-slate-700 transition-colors text-slate-300 hover:text-emerald-400 shadow-lg">
              <User className="w-[18px] h-[18px]" aria-hidden />
            </Link>

            <button type="button" aria-label="Log out" onClick={logout} className="bg-red-500/10 hover:bg-red-500/20 text-red-400 p-2.5 rounded-2xl border border-red-500/20 transition-all">
              <LogOut className="w-4 h-4" aria-hidden />
            </button>
          </div>
        ) : (
          <button type="button" onClick={login} className="bg-emerald-600 hover:bg-emerald-500 px-6 py-2.5 rounded-2xl text-sm font-bold transition-all shadow-lg shadow-emerald-500/20">Log In</button>
        )}
      </nav>

      <main className="flex flex-col items-center mt-4 px-4 max-w-4xl mx-auto">
        ...
        <div className="w-full max-w-md bg-slate-800/50 border border-slate-700 p-8 rounded-2xl backdrop-blur-sm shadow-2xl relative z-10">
          {!authenticated ? (
            <button type="button" onClick={login} className="w-full bg-emerald-500 hover:bg-emerald-400 py-3 rounded-xl font-bold">Connect Wallet</button>
          ) : (
            <div className="flex flex-col gap-4">
              <div className="bg-slate-900/80 p-1 rounded-xl flex mb-2 border border-slate-700 overflow-x-auto no-scrollbar" role="tablist">
                {(
                  [
                  ...
                  <button key={key} type="button" role="tab" aria-selected={mode === key} onClick={() => setMode(key)} className={`flex-1 min-w-max px-3 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold transition-all ${mode === key ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>
              ...
              {mode === 'crypto' && (
                <>
                  <div className="relative">
                    <button type="button" aria-label="Select token" aria-expanded={isTokenListOpen} onClick={() => setIsTokenListOpen((v) => !v)} className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 flex justify-between items-center hover:border-slate-600 transition-all">
                      <div className="flex items-center gap-2">
                        <div className={`w-5 h-5 rounded-full ${selectedAsset.icon} flex items-center justify-center text-[8px]`} aria-hidden>{selectedAsset.symbol[0]}</div>
                    ...
                    {isTokenListOpen && (
                      <div className="absolute top-full w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl z-20 overflow-hidden shadow-xl">
                        {ASSETS.map((a, index) => (
                          <div key={a.symbol} role="option" aria-selected={selectedAssetIndex === index} tabIndex={0} onClick={() => { setSelectedAssetIndex(index); setIsTokenListOpen(false); }} className="p-3 hover:bg-slate-700 cursor-pointer flex gap-3">
                            <div className={`w-6 h-6 rounded-full ${a.icon} flex items-center justify-center text-[10px]`} aria-hidden>{a.symbol[0]}</div>
                            {a.name}
                          </div>
                        ))}
                      </div>
                  ...
                  <div>
                    ...
                    <input id="crypto-seller" value={sellerAddress} onChange={(e) => setSellerAddress(e.target.value)} placeholder="0x… or seller@email.com" autoComplete="off" className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 mt-1 outline-none focus:border-emerald-500 transition-all" />
                  </div>
                  <div>
                    <label htmlFor="crypto-amount" className="text-xs text-slate-400 ml-1 font-bold">AMOUNT</label>
                    <input id="crypto-amount" type="number" min="0" step="any" value={amountInput} onChange={(e) => setAmountInput(e.target.value)} placeholder="0.00" className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 mt-1 outline-none focus:border-emerald-500 transition-all" />
                  </div>
                  <button type="button" onClick={handleCryptoTransaction} disabled={isWriting || (!isUnsupportedNetwork && (!sellerAddress.trim() || !amountInput))} className={`w-full py-4 rounded-xl font-bold mt-2 flex items-center justify-center gap-2 transition-all ${isUnsupportedNetwork ? 'bg-red-600 hover:bg-red-500 text-white' : 'bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white'}`}>
                ...
                <>
                  {!hasEmailLinked ? (
                    <div className="flex flex-col items-center justify-center p-6 bg-slate-900/50 border border-slate-700 rounded-2xl text-center gap-4 mt-2">
                      <div className="w-14 h-14 bg-blue-500/20 border border-blue-500/30 rounded-full flex items-center justify-center mb-2"><Mail className="w-6 h-6 text-blue-400" aria-hidden /></div>
                      <h3 className="text-lg font-bold text-white">Email Verification Required</h3>
                      <p className="text-sm text-slate-400 leading-relaxed">To use Bank Transfer escrows, you must link an email address to your account.</p>
                      <button type="button" onClick={linkEmail} className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-xl mt-2 transition-all w-full flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20">Link Email Address <ArrowRight className="w-4 h-4" aria-hidden /></button>
                    </div>
                  ) : (
                    <>
                      <div className="grid grid-cols-3 gap-2">
                        <div className="col-span-1 bg-slate-900/50 border border-slate-700 rounded-lg px-3 py-3 flex items-center justify-center gap-2 cursor-not-allowed opacity-80">
                          <span className="text-sm font-bold">NGN</span><div className="w-4 h-4 rounded-full bg-green-600 flex items-center justify-center text-[8px] text-white">₦</div>
                        </div>
                        <div className="col-span-2">
                          <label htmlFor="fiat-amount" className="sr-only">Amount in NGN</label>
                          <input id="fiat-amount" type="number" min="0" step="any" value={fiatAmount} onChange={(e) => setFiatAmount(e.target.value)} placeholder="Amount (e.g. 5000)" className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 outline-none focus:border-blue-500 transition-all" />
                        </div>
                      </div>
                      <div className="opacity-70 cursor-not-allowed">
                        <label htmlFor="fiat-buyer-email" className="text-xs text-slate-400 ml-1 font-bold flex items-center gap-1.5">YOUR EMAIL <Lock className="w-3 h-3 text-slate-500" aria-hidden /></label>
                        <input id="fiat-buyer-email" readOnly value={buyerEmail} autoComplete="email" className="w-full bg-slate-900/80 border border-slate-700 rounded-lg px-4 py-3 mt-1 outline-none text-slate-400 cursor-not-allowed" />
                      </div>
                      <div>
                        <label htmlFor="fiat-seller-email" className="text-xs text-emerald-400 ml-1 font-bold">SELLER'S TRUSTLINK EMAIL</label>
                        <input id="fiat-seller-email" type="email" autoComplete="email" value={sellerEmail} onChange={(e) => setSellerEmail(e.target.value)} placeholder="seller@email.com" className="w-full bg-slate-900/50 border border-emerald-500/30 rounded-lg px-4 py-3 mt-1 outline-none focus:border-emerald-500 transition-all" />
                      </div>
                      <div>
                        ...
                        <div className="flex flex-col gap-2 mt-1">
                          ...
                          <select id="fiat-bank" value={bankCode} onChange={(e) => setBankCode(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 outline-none focus:border-blue-500 transition-all text-sm appearance-none">
                          ...
                          <div className="relative">
                            ...
                            <input id="fiat-acct-no" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={10} autoComplete="off" value={accountNumber} onChange={(e) => setAccountNumber(e.target.value.replace(/\D/g, ''))} placeholder="Account Number (10 digits)" className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 outline-none focus:border-blue-500 transition-all" />
                            {isResolving && <div className="absolute right-4 top-3.5"><Loader2 className="animate-spin w-4 h-4 text-blue-500" aria-label="Verifying account…" /></div>}
                          </div>
                          <div aria-live="polite" className={`w-full bg-slate-800/50 border ${accountName ? 'border-emerald-500/30 bg-emerald-500/10' : resolveError ? 'border-red-500/30 bg-red-500/10' : 'border-slate-800'} rounded-lg px-4 py-3 transition-all flex items-center gap-2 min-h-[46px]`}>
                            {accountName ? <><div className="bg-emerald-500 rounded-full p-0.5"><CheckCircle2 className="w-3 h-3 text-white" aria-hidden /></div><span className="text-xs font-bold text-emerald-400 tracking-wide">{accountName}</span></> : resolveError ? <><div className="bg-red-500 rounded-full p-0.5"><X className="w-3 h-3 text-white" aria-hidden /></div><span className="text-xs font-bold text-red-400 tracking-wide">{resolveError}</span></> : <span className="text-xs text-slate-600 italic flex items-center gap-2"><UserCheck className="w-3 h-3" aria-hidden /> Account Name will appear here</span>}
                          </div>
                        </div>
                      </div>
                      <div>
                        <label htmlFor="fiat-description" className="text-xs text-slate-400 ml-1 font-bold">DESCRIPTION</label>
                        <textarea id="fiat-description" value={fiatDescription} onChange={(e) => setFiatDescription(e.target.value)} placeholder="What are you paying for?" maxLength={500} className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 mt-1 outline-none focus:border-blue-500 transition-all h-24 resize-none" />
                      </div>
                      <button type="button" onClick={handleFiatTransaction} disabled={!fiatAmount || !accountName || !buyerEmail || !sellerEmail} className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-bold mt-2 disabled:opacity-50 flex items-center justify-center gap-2">Secure Bank Transfer <ArrowRight className="w-4 h-4" aria-hidden /></button>
                    </>
                  )}
                </>
              )}
              ...
              {mode === 'giftcard' && (
                <div className="relative mt-2">
                  <div className={`flex flex-col gap-4 transition-all duration-300 ${isGiftCardLocked ? 'blur-md pointer-events-none opacity-40 select-none' : ''}`} aria-hidden={isGiftCardLocked}>
                    <div>
                    ...
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center p-6 bg-slate-900/40 rounded-xl">
                      {trustLevel === null ? (
                        ...
                        <>
                          ...
                          <p className="text-sm text-slate-300">Gift Card trading is locked to prevent fraud. Complete more standard trades to reach Trust Level 3 and unlock this feature.</p>
                        </>
                      )}
                    </div>
                  )}
          ...
          <div className="w-full mt-20 border-t border-white/10 pt-10">
            <div className="flex justify-between items-end mb-6 border-b border-white/10 pb-1">
              <div className="flex gap-6" role="tablist">
                <button type="button" role="tab" aria-selected={dashboardTab === 'buying'} onClick={() => setDashboardTab('buying')} className={`text-lg font-bold pb-4 border-b-2 transition-all ${dashboardTab === 'buying' ? 'border-emerald-400 text-emerald-400' : 'border-transparent text-slate-500'}`}>I'm Buying</button>
                <button type="button" role="tab" aria-selected={dashboardTab === 'selling'} onClick={() => setDashboardTab('selling')} className={`text-lg font-bold pb-4 border-b-2 transition-all ${dashboardTab === 'selling' ? 'border-blue-400 text-blue-400' : 'border-transparent text-slate-500'}`}>I'm Selling</button>
              </div>

              <div className="flex items-center gap-3 pb-3">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-800/50 px-3 py-1 rounded-full border border-slate-700">Showing {mode === 'crypto' ? 'Cryptocurrency' : mode === 'fiat' ? 'Bank Transfer' : 'Gift Card'} Orders</span>
                <button type="button" aria-label="Refresh orders" onClick={handleRefresh} className="text-slate-500 hover:text-white"><RefreshCcw className="w-4 h-4" aria-hidden /></button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {displayedOrders.length === 0 && (
                <div className="text-slate-500 text-center py-10 italic border border-dashed border-slate-700 rounded-xl">
                  No active <span className="capitalize">{mode === 'crypto' ? 'Cryptocurrency' : mode === 'fiat' ? 'Bank Transfer' : 'Gift Card'}</span> orders found.
                </div>
              )}
            </div>
          </div>
